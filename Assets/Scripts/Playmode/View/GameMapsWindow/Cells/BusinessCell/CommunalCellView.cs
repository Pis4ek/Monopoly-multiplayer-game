...
{
    public class CommunalCellView : BaseBusinessCellView
    {
        ...
        private void UpdateHeader()
        {
            ...
            {
                _headerText.text = $"x{_cellBData.Config.IncomeByLevel[0]}";
                return;
            }
            var count = _data.MapData.GetPlayersCellsCountByType(_cellBData.Owner, _cellBData.BusinessType);
            _headerText.text = $"x{_cellBData.Config.IncomeByLevel[count]}";
        }
    }
}